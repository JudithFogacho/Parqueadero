...
// -----------------------------------------------------------------
    ...
    {
        boolean ocupado = puestos.get( pPuesto).estaOcupado( );
        return ocupado;
    }
    ...
    public boolean hayCarroMasDeOchoHoras() {
        boolean carrosPasados8= false;
        int maxTiempo= 8;
        for (Puesto puesto: puestos){
            if (puesto.estaOcupado()){
                Carro carrito= puesto.darCarro();
                int tiempo= carrito.darTiempoEnParqueadero(horaActual);
                if (tiempo>maxTiempo){
                    carrosPasados8=true;
                }
            }
        }
        return carrosPasados8;
    }
    public ArrayList<Carro> darCarrosMasDeTresHorasParqueados( ) {
        ArrayList<Carro> carrito3Horas= new ArrayList<>();
        int limTiempo= 3;
        for (Puesto puesto: puestos){
            if(puesto.estaOcupado()){
                Carro carrito= puesto.darCarro();
                int tiempo= carrito.darTiempoEnParqueadero(horaActual);
                if (tiempo>limTiempo){
                    carrito3Horas.add(carrito);
                }
            }
        }
        return carrito3Horas;
    }
    public boolean hayCarrosPlacaIgual() {
        ArrayList<String> placas= new ArrayList<>();
        for (Puesto puesto: puestos){
            String plaquita= puesto.darCarro().darPlaca();
            if (placas.contains(plaquita)){
                return true;
            }else{
                System.out.println("La placa no es igual");
            }
        }
        return false;
    }
    public int carrosPlacaPB (){
        ...
        for(Puesto puesto: puestos){
            if (puesto.estaOcupado()){
                Carro carrito =puesto.darCarro();
                String placaPB= carrito.darPlaca();
                if (placaPB.startsWith("PB")){
                    contador++;
                }
            }
        }
    ...
    {
        int cantidadCarritosPB= carrosPlacaPB();
        boolean carro24Horas = carroMas24Horas();
        String mensaje= "Cantidad de Carros PB "+cantidadCarritosPB+
